...
ALIASES={'PAÍSES BAJOS':'PAISES BAJOS','MÉXICO':'MEXICO','IRÁN':'IRAN','CANADÁ':'CANADA','BÉLGICA':'BELGICA','ESPAÑA':'ESPANA','JAPÓN':'JAPON','REPÚBLICA CHECA':'REPUBLICA CHECA','TÚNEZ':'TUNEZ','SUDÁFRICA':'SUDAFRICA','HAITÍ':'HAITI','TURQUÍA':'TURQUIA','UZBEKISTÁN':'UZBEKISTAN','PANAMÁ':'PANAMA','UNITED STATES':'ESTADOS UNIDOS','USA':'ESTADOS UNIDOS','NETHERLANDS':'PAISES BAJOS','IVORY COAST':'COSTA DE MARFIL','CZECHIA':'REPUBLICA CHECA','DR CONGO':'RD CONGO','SAUDI ARABIA':'ARABIA SAUDITA'}
...
def canon_country_name(v):
    ...
    if isinstance(v,dict):
        for k in ('name','nombre','team','equipo','short_name','shortName','displayName','title'):
            if v.get(k): return canon_country_name(v[k])
        return ''
    s=re.sub(r'\s+',' ',str(v).replace('\xa0',' ').strip())
    if not s: return ''
    s=re.sub(r'^[\U0001F1E6-\U0001F1FF]{2}\s*','',s).strip()
    u=s.upper().strip()
    if u.startswith(('GANADOR','PERDEDOR')): return u
    if u.startswith('RIVAL') or u.startswith('MEJOR TERCERO') or re.fullmatch(r'[123][A-L](?:/[A-L])*',u): return ''
    m=re.match(r'^([A-Za-z]{2})\s+(.+)$',s)
    if m and m.group(1).upper() in PREFIXES: s=m.group(2).strip()
    u=s.upper().strip()
    if u in ALIASES: return ALIASES[u]
    return ALIASES.get(strip_accents(u),strip_accents(u))
def display_country(v):
    c=canon_country_name(v)
    if not c: return ''
    if c.startswith(('GANADOR','PERDEDOR')): return c
